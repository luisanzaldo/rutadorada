...
- El **panel admin** (`/admin`) permite crear y editar posts desde el navegador: el contenido se commitea al repo vía la API de GitHub y GitHub Actions construye y despliega. La sesión del admin usa JWT (`jose`) + bcrypt; los usuarios se definen en `src/lib/users.ts` con hashes en variables de entorno.
...
- `rss-state.json` lo actualiza una automatización externa; el `paths-ignore` del workflow evita que esos commits disparen builds.
